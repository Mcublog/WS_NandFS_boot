...
uint32_t console_cmd_set_size_and_end(console_cmd_t* cmd, uint8_t *b)
{
    ...
    *((uint32_t*)&b[crc_idx])=0;
    //size_pkt=size-CL_SIZE_START_STOP_SYMB-CL_SIZE_END_SYMB-CRC32_LENGHT;
    crc = xcrc32 (b, size-CL_SIZE_START_STOP_SYMB-CL_SIZE_END_SYMB-CRC32_LENGHT, 0xffffffff);//crc без стоповых символов и самой crc
                          ...
                          uint8_t *b)
{
    ...
    {
        for (i=0;i<num;i++) p=set_cmd_comp(param[i].data, &cmd->param.p[i], p);
    }
    else
    {
        for (i=0;i<num;i++) p=set_cmd_comp_bin(&cmd->param.p[i], param[i].data, param[i].size, p);
